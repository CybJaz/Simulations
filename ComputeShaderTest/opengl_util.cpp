...
unsigned int texture_numbers = 0;

GLuint genRenderProg(GLuint texHandle, GLuint& vao_handle) {
...
GLuint genTexture(GLuint &texHandle) {
	// We create a single float channel 512^2 texture
	//GLuint texHandle;
	glGenTextures(1, &texHandle);

	//if (texture_numbers == 0)
	//{
	//	std::cout << std::hex << GL_TEXTURE0 << std::dec << std::endl;
	//	glActiveTexture(GL_TEXTURE0);
	//}
	//else
	//{
	//	std::cout << std::hex << GL_TEXTURE1 << std::dec << std::endl;
	//	glActiveTexture(GL_TEXTURE1);
	//}
	glActiveTexture(GL_TEXTURE0 + texture_numbers);
	...
	glBindImageTexture(texture_numbers, texHandle, 0, GL_FALSE, 0, GL_READ_WRITE, GL_RGBA16F);
	//glBindImageTexture(0, texHandle, 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_R32F);

	std::cout << "tex gen " << texture_numbers << " " << texHandle << std::endl;
